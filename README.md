...
- 🖥️ 界面设计：[docs/dashboard-mvp.md](docs/dashboard-mvp.md)
- 🚀 快速上手：[docs/getting-started.md](docs/getting-started.md)
- 📚 示例案件：[examples/README.md](examples/README.md)

## ✨ 核心卖点

| 方向 | RegentOS 关注的不是 | RegentOS 真正关注的是 |
|------|----------------------|------------------------|
| ⚖️ 治理 | Agent 如何更快协作 | 谁起草、谁审批、谁监督、谁建议修复、谁授权修复 |
| 🧾 流程 | 任务是否直接跑完 | 任务是否被案件化、审批化、卷宗化 |
| 🛡️ 风险 | 出错后如何快速重试 | 出错后谁记录、谁诊断、谁批准修复 |
| 🖥️ 展示 | 单一任务看板 | 文渊阁主看板 + 独立锦衣卫看板 |
...
```mermaid
flowchart LR
    U["👤 用户<br/>Original Task"] --> TG["📨 通政司<br/>案件化 / 编号 / 元信息"]
    TG --> CB["📝 内阁<br/>票拟 / 拆解 / 复审"]
    CB --> SJ["🟥 司礼监<br/>审批 / 封驳 / 批令"]
    SJ --> D{"审批决策"}

    D -- "通过" --> EX["⚙️ 动态执行层<br/>分发任务 / 执行 / 工具调用"]
    D -- "封驳" --> RW["↩️ 返工 / 补证<br/>rejected -> reworking"]
    RW --> CB

    EX --> TY["🟨 台谏院<br/>全过程监督"]
    TY -. "跳步 / 越权 / 资源异常" .-> JW["🕵️ 锦衣卫<br/>问题定位 / 影响分析 / 修复建议"]
    EX -. "执行异常" .-> JW

    JW --> UD{"用户是否授权修复?"}
    UD -- "是" --> RO["🛠️ 修复受令<br/>repair_authorized -> rerunning -> executing"]
    UD -- "否" --> HOLD["❄️ 冻结 / 终止"]

    EX --> RP["📜 回奏反馈<br/>reporting -> archived"]
    RO --> RP

    classDef intake fill:#d9ebff,stroke:#2b6cb0,color:#17324d,stroke-width:1.5px;
    classDef planning fill:#d9f7e8,stroke:#2f855a,color:#143224,stroke-width:1.5px;
    classDef approval fill:#ffe0db,stroke:#c53030,color:#4a1d1d,stroke-width:1.5px;
    classDef execution fill:#e5f4ff,stroke:#3182ce,color:#18324d,stroke-width:1.5px;
    classDef oversight fill:#fff1bf,stroke:#b7791f,color:#4a3514,stroke-width:1.5px;
    classDef repair fill:#efe1ff,stroke:#7b3fe4,color:#34214a,stroke-width:1.5px;
    classDef neutral fill:#f7f3ee,stroke:#8b7d6b,color:#2d241d,stroke-width:1.2px;

    class TG intake;
    class CB planning;
    class SJ approval;
    class EX,RP execution;
    class TY oversight;
    class JW,RO repair;
    class U,D,RW,UD,HOLD neutral;
```

### 🖥️ 看板如何对应这条主线

- 🏛️ `文渊阁主看板`：聚焦案件状态流转、卷宗时间线、审批与回奏主线。
- 🕵️ `锦衣卫看板`：聚焦执行过程中暴露的内部问题、等级划分、修复建议与证据事件。
...
- 🏛️ 文渊阁主看板：`Kanban + Memorials`
- 🕵️ 锦衣卫看板：内部问题、等级划分、修复建议
- ✅ 一条完整治理链从创建走到归档
